document.addEventListener('DOMContentLoaded', () => {
    ...
    const userParam = urlParams.get('user') || urlParams.get('username') || urlParams.get('nome');
    if (userParam) {
    ...
    function resizeTextToFit() {
        ...
        const maxWidth = parseFloat(styles.width) - parseFloat(styles.paddingLeft) - parseFloat(styles.paddingRight);
        
        // Safety loop to prevent infinite shrink if something is wrong
        let iterations = 0;
        while (usernameEl.scrollWidth > maxWidth && fontSize > 0.5 && iterations < 50) {
            fontSize -= 0.1;
            usernameEl.style.fontSize = fontSize + 'rem';
            iterations++;
        }
